...
- **解释推荐：** 展开“匹配详情”，分别查看标题、摘要、必要语境与排除规则的命中情况。
- **离线调参：** 在固定候选集上预览新配置，先看推荐变化，再决定是否应用到论文库。
...
这组**六条虚构候选**演示了规则变化：新配置加入桌面自动化候选，移出化学试剂与医疗诊断候选。预览不联网、不调用模型，也不修改配置或论文库；这些结果只是规则示例，不代表推荐准确率提高。

处理自己的论文库时，先用 `export-candidates` 导出已存储的候选，包括未推荐和已隐藏的论文，不包含阅读笔记或模型解读，再比较新旧配置。排除词采用字面短语匹配，否定句里出现也会触发，需要检查误排。[查看调参流程 →](docs/profile-tuning.md)
...
“匹配详情”说明主题为何匹配或被阻断。被当前规则排除的已管理论文仍保留阅读记录，并显示排除徽章。命中短语是规则解释，不是论文原文引文或相关性的证明。
